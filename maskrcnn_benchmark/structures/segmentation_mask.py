...
class BinaryMaskList(object):
    ...
    def area(self):
        ...
        return [np.sum(mask) for mask in masks]
...
class PolygonInstance(object):
    ...
    def area(self):
        ...
        cropped = self.crop([x1, y1, x2, y2])
        polygons = [p.numpy() for p in cropped.polygons]
        rles = mask_utils.frPyObjects(polygons, y2-y1, x2-x1)
        return np.sum(mask_utils.area(rles))
...
class PolygonList(object):
    ...
    def area(self):
        return [polygon.area() for polygon in self.polygons]
